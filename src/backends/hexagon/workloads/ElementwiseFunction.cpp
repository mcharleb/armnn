...
{
	hexagon::BroadcastLoop(inShape0, inShape1, outShape).Unroll(Functor(), 0, inData0, inData1, outData);
}
...
template struct armnn::ElementwiseFunction<armnn::hexagon::maximum<float>, float, float>;
template struct armnn::ElementwiseFunction<armnn::hexagon::minimum<float>, float, float>;
